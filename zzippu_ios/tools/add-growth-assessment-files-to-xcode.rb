#!/usr/bin/env ruby
# tools/add-growth-assessment-files-to-xcode.rb
# 성장 판정(기대 체중·키 + 5카테고리 백분위) 슬라이스의 신규 파일을 zzippu 타겟에 등록.
# .swift → source build phase, .json(키 밴드 데이터) → resources build phase.
# Usage: ruby tools/add-growth-assessment-files-to-xcode.rb
...
RESOURCE_FILES = %w[
  Shared/Resources/Guidelines/who_growth_height_boy.json
  Shared/Resources/Guidelines/who_growth_height_girl.json
].freeze
