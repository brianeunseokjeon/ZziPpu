...
### 2.2 MainTabView (하단 TabView) — **4탭. 상세는 `IA_4TAB.md`가 정본**
> 구 5탭(+AI 세그먼트)안은 폐기. 확정 IA는 **홈/대시보드/발달/설정 4탭**이며 화면 구성·매핑·개발순서는 **`IA_4TAB.md`** 참조.

| Tab | SF Symbol | 화면 | MVP |
|-----|-----------|------|-----|
| 홈(기록) | `house.fill` | 기록 허브 + 오늘 타임라인 | ✅ |
| 대시보드 | `heart.text.square.fill` | **요약 카드→상세 차트(대시보드+추세 통합, 건강앱식)** + 성장곡선 | ✅ |
| 발달 | `figure.child` | 연령별 발달·이정표(번들) + 예방접종 | ✅ |
| 설정 | `gearshape.fill` | 아기 프로필·공동양육 공유·계정·내보내기 | ✅ |

- 변경 핵심: **추세 탭 삭제(대시보드로 통합)**, **AI 4종 전부 미노출**(후순위), 예방접종→발달, 성장→대시보드 상세.
- 상단 헤더(아바타+이름/나이+날짜네비 `selectedDate`)는 **홈·대시보드**에만 공통 표시.
