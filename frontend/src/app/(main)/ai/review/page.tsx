...
export default function AIReviewPage() {
  ...
  const generateMutation = useGenerateReview(activeBabyId, selectedDate);

  const selectedReview = reviews.find((r) => r.reviewDate === selectedDate);
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{formatKoreanDate(selectedDate)}</p>
              </div>
            </div>
            <Button
              size="sm"
      ...
      {/* Today Review */}
      {!generateMutation.isPending && selectedReview && (
        <DailyReviewCard review={selectedReview} />
      )}

      {/* Empty State */}
      {!generateMutation.isPending && !selectedReview && !reviewsLoading && (
        <Card>
          <CardContent className="pt-6">
            <div className="flex flex-col items-center gap-4 py-8 text-center">
