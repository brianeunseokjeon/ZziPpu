...
## ✅ 완료 (2026-06-06, commit a344db0)

### 대시보드 수유량 적정성 — 3가지 모두 완료
1. **체중 입력 버그 수정**: `WeightInline.recorded_at`을 `toISOString()` → `todayDateString()`(YYYY-MM-DD KST)로 수정. 422 해결.
2. **WeightInline 공용 컴포넌트화**: `features/growth/components/WeightInline.tsx`로 추출. 설정·대시보드 공유.
3. **설정 페이지 체중 입력**: 아기 정보 카드에 현재 체중 행 추가 (growth records SSOT).
4. **온보딩 출생 체중**: 출생 체중 입력 추가 → `baby.birth_weight_g` + 출생일자 growth record 동기화.
5. **AAP 출처 명시**: `feedingGuideline.ts` 주석에 AAP/HealthyChildren.org 출처, 면책 문구 강화.
